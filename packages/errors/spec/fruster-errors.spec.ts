import exampleErrors from "../examples/example";
...
describe("Fruster error", () => {
	it("should create errors", () => {
		...
		expect(error.error.detail).toBe(
			"I was hoping for Bernie, but Trump was elected"
		);

		const oError = exampleErrors.get("INVALID_PRESIDENT", "Hillary", "Trump");

		expect(oError.error.detail).toBe(
			"I was hoping for Hillary, but Trump was elected"
		);
	...
	it("should not fail to create details even though not all args was passed in", () => {
		...
		expect(error.error.detail).toBe(
			"I was hoping for Bernie, but undefined was elected"
		);
	});
		...
		const errors = [
			{
				...
				code: "DUPLICATE",
				title: "Foo",
				detail: (president) => `I was hoping for ${president}`,
			},
			{
				status: 500,
				code: "DUPLICATE",
	...
	it("should pass detail params as varargs", (done) => {
		// get
		const gottenError = exampleErrors.get(
			"INVALID_PRESIDENT",
			"Bernie",
			"Trump"
		);
		expect(gottenError.error.detail).toBe(
			"I was hoping for Bernie, but Trump was elected"
		);

		// reject
		exampleErrors
			.reject("INVALID_PRESIDENT", "Hillary", "Trump")
			.catch((rejectedError) => {
				expect(rejectedError.error.detail).toBe(
					"I was hoping for Hillary, but Trump was elected"
				);
				done();
			});
	...
	it("should have added default errors that does not already exist in error model", () => {
		...
		expect(exampleErrors.internalServerError().error.title).toBe(
			"Something broke"
		);
	});

	it("should create default errors", () => {
		const errors = frusterErrors();
		...
		expect(internalServerError.error.title).toBe(
